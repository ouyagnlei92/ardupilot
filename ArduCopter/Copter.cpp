...
void Copter::batterySmartRTLUpdate(void){
	...
	if(!flightmode->requires_GPS()) return; 

	++_log_record_time;
	if(_log_record_time>=5){  //1000ms
		_log_record_time = 0;
		writeLog();
	}
	...
	if(_mv_success){
		//�������
		float climb_rate1 = inertial_nav.get_velocity_z(); // cm/s
		float groundSpeed = ahrs.groundspeed()*100;    //  cm/s

		_up_flag = climb_rate1>=0 ? true:false;				    


		if(fabs(climb_rate1)>=50 && groundSpeed<=50){    
			climbUseMahCal();
		}else if(fabs(climb_rate1)<50 && groundSpeed>=50){ 
			horUserMahCal();
		}else if(fabs(climb_rate1)>=50 && groundSpeed>=50) { 
			horClimbUseMahCal();
		}else if(fabs(climb_rate1)<50 && groundSpeed<50){   
...
void Copter::horUserMahCal(void){    
	if(!_hormove_start){
		if(!_horMoving){
			_horMoving = true;
...
void Copter::horClimbUseMahCal(void){   
	if(!_horclimbe_start){   
		if(!_horclimbing){
			_horclimbing = true;
...
void Copter::stopUseMahCal(void){   
	if(!_stop_start){
		if(!_stopping){
			_stopping = true;
...
void Copter::lowPowerRTL(void){
	_to_home_distance = home_distance(); 
...
void Copter::smartBatteryAutoRTL(void){
	...
	if(motors->armed()){
		if(!_armed){
			init();
			_pre_arm_mah = battery.remaining_mah();   
			_armed = true;
			_mv_success = true;
