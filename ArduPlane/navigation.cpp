...
void Plane::wp_continue_fly()
{
	...
	AP_Mission& mission = this->mission;
	...
	if(mission.state()==AP_Mission::MISSION_RUNNING){  //�����������ڽ���
		...
		//�ж��Ƿ�ʼ����ϵ��¼
		if(arming.is_armed() && !wp_continue_start){
		...
		if(wp_continue_start){
			...
			if(AP_HAL::millis()-pos_last_time_ms>=pos_time_ms){ //��¼һ������λ��
			    if(nav_cmd1.id==MAV_CMD_NAV_WAYPOINT){
					...
					float curr_posdis = get_distance(current_loc, nav_cmd1.content.location);
					if(curr_posdis<=mission.wp_continue_pos_distance()*100){
						...
						mission_cmd[current_cmd_index].content.location = current_loc;  //��¼��ǰλ��
						++current_cmd_index;
						++pos_count;
					}						
					pos_last_time_ms = AP_HAL::millis();
	...
	//������ɲ����п������ϵ�����,�������к���,�ڼ�����ʱ�����
   if(!arming.is_armed() && wp_continue_complete && !is_flying() && mission.state()!=AP_Mission::MISSION_COMPLETE){
